...
def create_iam_role(iam, DWH_IAM_ROLE_NAME):
    ...
    params: 
        ...
        DWH_IAM_ROLE_NAME -- IAM Role name to be created   
    returns: 
...
def create_cluster(redshift, roleArn, DWH_CLUSTER_TYPE, DWH_NODE_TYPE, DWH_NUM_NODES, DWH_DB, DWH_CLUSTER_IDENTIFIER, DWH_DB_USER, DWH_DB_PASSWORD):
    ...
    params:
        redshift -- boto3 redshift client instance
        roleArn -- Name of the IAM Role with Redshift & S3 Access
        DWH_CLUSTER_TYPE -- Redshift cluster type
        DWH_NODE_TYPE -- Redshift node type
        DWH_NUM_NODES -- Number of nodes for the redshift cluster
        DWH_DB -- Redhsift DB Name
        DWH_CLUSTER_IDENTIFIER -- Redshift Cluster Name
        DWH_DB_USER -- Redshift User Name
        DWH_DB_PASSWORD -- Redshift Password Name
    returns:None
    '''
...
def get_cluster_props(redshift, DWH_CLUSTER_IDENTIFIER):
    ...
    params: 
        redshift -- boto3 redshift client instance
        DWH_CLUSTER_IDENTIFIER -- Name of the Redshift cluster
    returns :
        myClusterProps -- A dict with cluster properties
        DWH_ENDPOINT -- Redshift host name
        DWH_ROLE_ARN -- IAM Role created for accessing redshift
    '''
...
def open_ports(ec2, myClusterProps, DWH_PORT):
    ...
    params: 
        ec2 -- Resource Instance
        myClusterProps -- A dictionary with required cluster properties of the Redshift cluster
        DWH_PORT -- Port to be connected to
    returns : None
    '''
...
def main():
    """
    Reads the config file & triggers methods to create aws infra & saves the updated info back
    params: None
    returns: None
    """
    ...
    print('Saving config file with updated info')
